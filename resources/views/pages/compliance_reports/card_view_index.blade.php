@extends('layouts.app')

@section('app_css')
    {!! $cdv_compliance_reports->render_css() !!}
@stop

@section('title_postfix')
Compliance Reports
@stop

@section('page_title')
Compliance Reports
@stop

@section('page_title_suffix')
All Compliance Reports
@stop

@section('page_title_subtext')
<a class="ms-1" href="{{ route('dashboard') }}">
    <i class="bx bx-chevron-left"></i> Back to Dashboard
</a>
@stop

@section('page_title_buttons')
<a id="btn-new-mdl-complianceReport-modal" class="btn btn-sm btn-primary btn-new-mdl-complianceReport-modal">
    <i class="bx bx-book-add me-1"></i>New Compliance Report
</a>
@if (Auth()->user()->hasAnyRole(['','admin']))
    @include('socla-apply-module::pages.compliance_reports.bulk-upload-modal')
@endif
@stop

@section('content')
    <div class="row row-cols-1 row-cols-md-2 row-cols-xl-4 hidden-sm hidden-xs">
        {{-- Summary Row --}}
    </div>
    
    <div class="card border-top border-0 border-4 border-primary">
        <div class="card-body">
            {{ $cdv_compliance_reports->render() }}
        </div>
    </div>

    @include('socla-apply-module::pages.compliance_reports.modal')
@stop

@section('side-panel')
<div class="card radius-5 border-top border-0 border-4 border-primary">
    <div class="card-body">
        <div><h5 class="card-title">More Information</h5></div>
        <p class="small">
            This is the help message.
            This is the help message.
            This is the help message.
        </p>
    </div>
</div>
@stop

@push('page_scripts')
    {!! $cdv_compliance_reports->render_js() !!}
@endpush